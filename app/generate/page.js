...
import { Box, Fab, Button, Card, CardActionArea, CardContent, Container, Dialog, DialogActions, DialogContent, DialogContentText, DialogTitle, Grid, Paper, TextField, Typography } from "@mui/material"
...
export default function generate() {
    ...
    const handleSubmit = async () => {
        fetch('api/generate', {
            ...
            body: JSON.stringify({ content: text }) // Wrap the input text in a JSON object
        })
            .then((res) => res.json())
            .then((data) => {
                setFlashcards(data);
    ...
    const saveFlashcards = async () => {
        ...
        if (!user) {
            userDocRef = doc(collection(db, "users"), user.id); // Replace "userId" with actual user ID
        }
        else {
    ...
    return (
        <Container maxWidth='md'>
            <Fab variant="contained" sx={{mt:2}} onClick={() => {router.push('/')}}>
                back
            </Fab>
            <Box
                sx={{
